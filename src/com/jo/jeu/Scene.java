...
import com.jo.affichage.CompteARebours;
import com.jo.affichage.Score;
import com.jo.audio.Audio;
import com.jo.objets.Bloc;
import com.jo.objets.Objet;
import com.jo.objets.Piece;
import com.jo.objets.TuyauRouge;
import com.jo.personnages.Champ;
import com.jo.personnages.Mario;
import com.jo.personnages.Tortue;

@SuppressWarnings("serial")
public class Scene extends JPanel {
	
	private ImageIcon icoFond;// Icon qu'on transformera en image des fond
	private Image imgFond1;
	private Image imgFond2;
	
	private ImageIcon icoChateau1;  // Icon qu'on transformera en image des objet
	...
	private int xFond1; // Position du fond
	private int xFond2;
	...
	private boolean ok; // Permet de ne jouer qu'une fois la musique de fin
	
	private int ySol; // Position (hauteur) du sol
	...
	private ArrayList<Objet> tabObjets; // Tableau dans lequel on mettra tous les bloc et les tuyau
	private ArrayList<Piece> tabPieces; // Tableau dans lequel on mettra les pieces, car elles n'ont pas le même comportement que les autres objets
	...
	public Scene() {	
		...
		mario = new Mario(300, 245); // On instancie l'objet (personnages) Mario, champ et tortue
		
		tuyauRouge1 = new TuyauRouge(600, 230); // On instancie les blocs, les tuyaux et les pieces
		...
		tabObjets = new ArrayList<Objet>();	// On instancie le tableau puis on y ajoute tous les tuyau et blocs
	...
	public void paintComponent (Graphics g) {
		...
		// Détection de contact entre les personnages et les objets
		...
		// Detection des contacts entre mario et les pieces
 	 	...
 	 	// Detection des contacts entre champ et tortue
		...
		// Déplacement du fond et des objet avec le fond
		...
		g2.drawImage(imgChateau1, 10 - this.xPos, 95, null); // Dessin du chateau et du panneau
		g2.drawImage(imgDepart, 220 - this.xPos, 234, null);		
 	 	for(int i = 0; i < this.tabObjets.size(); i++){ // Dessin des objets
 	 		g2.drawImage(this.tabObjets.get(i).getImgObjet(), this.tabObjets.get(i).getX(), this.tabObjets.get(i).getY(), null);
 	 	} 
 	 	for(int i = 0; i < this.tabPieces.size(); i++){ // Dessin des pieces
 	 		g2.drawImage(this.tabPieces.get(i).bouge(), this.tabPieces.get(i).getX(), this.tabPieces.get(i).getY(), null);
 	 	} 
 	 	g2.drawImage(imgDrapeau, 4650 - this.xPos, 115, null);// Dessin du drapeau d'arrivée 	    
 		g2.drawImage(imgChateauFin, 5000 - this.xPos, 145, null); // Dessin du chateau d'arrivée
 		
 		if(this.mario.isVivant() == true) { // Desssin de mario avec les déplacement (marche et saut) et sa mort
 			if(this.mario.isSaut()) { 
 	 			g2.drawImage(this.mario.saute(), this.mario.getX(), this.mario.getY(), null);
 	 		}
 	 		else{
 	 			g2.drawImage(this.mario.marche("mario", 50), this.mario.getX(), this.mario.getY(), null);
 	 		} 
 		}
 		else {
 			g2.drawImage(this.mario.meurt(), this.mario.getX(), this.mario.getY() - 6, null);
 		} 			
 		
 	 	for(int i = 0; i < this.tabChamps.size(); i++){ // Dessin des champ
 	 		if(this.tabChamps.get(i).isVivant() == true){ //  Seulement si le champ est vivant
 	    ...
 	    for(int i = 0; i < this.tabTortues.size(); i++){ // Dessin des tortues
 	 		if(this.tabTortues.get(i).isVivant() == true){ //  Seulement si la tortue est vivant
 	    ...
 	    g2.setFont(police); // Dessin de l'affichage du score et du compte à rebours
	    ...
	    if(this.finDePartie() == true){ // Fin de partie
